...
/** Reads MID prose texts in TEI markup using three nested `div`
* elements for citation units.
*
* @param editionType Type of edition to read.
*/
case class MidTeiReader(applicableType: MidEditionType) extends MidMarkupReader {
  require(recognizedTypes.contains(applicableType), "Unrecognized edition type: " + applicableType)


  def  recognizedTypes: Vector[MidEditionType] = MidTeiReader.recognizedTypes
  ...
  */
  def editedNodeCex(cn: CitableNode): String = {
    ...
    editionType match {
      case MidDiplomaticEdition => cex.append(MidTeiReader.diplomatic(archival))
    }
    cex.toString
  }
  ...
  def editedNode(cn: CitableNode): CitableNode = {
    ...
    val content = editionType match {
      case MidDiplomaticEdition => MidTeiReader.diplomatic(archival)
    }
...
object MidTeiReader {
  ...
  def readers : Vector[MidTeiReader] = {
